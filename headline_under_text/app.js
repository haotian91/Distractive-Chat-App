...
io.on('connection', function(socket){
    ...
    /*------new york times $ keywords-------*/ 
        socket.on('msgdone', function(data){ 
            console.log('msgdone:'+ data);
            var myText = data;
            alchemyapi.keywords("text", myText, {'sentiment': 1}, function(response){ 
                ...
                if (words>0){
                    var dataset;
                    fs.readFile('public/projection/data/texts.txt', function(err, data) {
                        if (err) {
                            console.log(err);
                        } else {
                            // console.log(data);
                            dataset = data.toString(); // convert to string
                            dataset = dataset + "," + keyWords[num].text;
                            console.log('dataset: ' + dataset); // Right!
                            // console.log('keyWords: ' + keyWords[num].text);

                        };

                        fs.writeFile('public/projection/data/texts.txt', dataset ,function(err){
                            console.log('writedata: '+dataset);
                            if(err){
                                console.log('error');
                            }else{
                ...
                // fs.writeFileSync('public/projection/data/texts.txt', keyWords[num].text);
                /*---------file system-----------*/
